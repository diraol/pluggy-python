...
class PixQrPaymentRecipient(BaseModel):
    """
    Payment recipient inferred from a PIX QR code (only the merchant name is exposed).
    """ # noqa: E501
    type: StrictStr
    name: Optional[StrictStr] = Field(default=None, description="Merchant name decoded from the PIX QR code")
    __properties: ClassVar[List[str]] = ["type", "name"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PIX_QR_CODE']):
            raise ValueError("must be one of enum values ('PIX_QR_CODE')")
        return value
